...
sys.path.append(os.path.join(CURRENT_DIR, 'location_service'))
sys.path.append(os.path.join(CURRENT_DIR, 'user_communication'))
sys.path.append(os.path.join(CURRENT_DIR, 'rating_system'))
...
DB_PATH = os.path.abspath(os.path.join(CURRENT_DIR, '..', 'database', 'laso_app.db'))
...
@app.route('/')
def root():
    return send_from_directory(os.path.abspath(os.path.join(CURRENT_DIR, '..', 'frontend')), 'index.html')

# Serve static frontend folder
@app.route('/frontend/<path:filename>')
def serve_frontend(filename):
    return send_from_directory(os.path.abspath(os.path.join(CURRENT_DIR, '..', 'frontend')), filename)
...
def get_provider_details(provider_id):
    ...
    # 1. Fetch provider details — try by provider_id first, then by user_id
